...
Offset = {}              # AE별 global offset value
...
def data_receiving():
    ...
    if isReady: #only send data if data is ready
        ...
        Displacement_ch4 = dis_conversion(rcv4[8:12]) + Offset(['dp'])
        Displacement_ch5 = basic_conversion(rcv4[12:]) + Offset(['dp'])
...
def set_config_data(config_data):
    ...
    # Revise latter!!!
    results['samplingRate'] =   int(np.uint16(100))           # hw fix 5/9
    results['sensingDuration'] = int(np.uint16(12*60*60))     # hw fix 5/9
    results['measurePeriod'] =  int(np.uint16(1))             # SC support 5/9 
    results['uploadPeriod'] =   int(np.uint16(6))             # hSC support 5/9
    results['sensorSelect'] =   int(np.uint16(ttp|tdp|tti|tac))
    results['highTemp'] =       int(np.int16(temp_config['ctrigger-tp']['st1high']*100))
    results['lowTemp'] =        int(np.int16(temp_config['ctrigger-tp']['st1low']*100))
    results['highDisp'] =       int(np.int16(temp_config['ctrigger-di']['st1high']*100))
    results['lowDisp'] =        int(np.int16(temp_config['ctrigger-di']['st1low']*100))
    results['highStrain'] =     int(np.int16(0))
    results['lowStrain'] =      int(np.int16(0))
    results['highTilt'] =       int(np.int16(temp_config['ctrigger-ti']['st1high']*100))
    results['lowTilt'] =        int(np.int16(temp_config['ctrigger-ti']['st1low']*100))
    results['highAcc'] =        int(np.uint16(temp_config['ctrigger-ac']['st1high']/0.0039/16))
    results['lowAcc'] =         int(np.int16(0))       # hw fix 5/9
    # end of formatting 
    return results 
