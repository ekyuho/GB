...
# 삭제할 파일 list 생성
def delete_filepath_list(aename, cleanperiod): # 가장 최근 rawperiod간의 파일을 뽑는다
    ...
    for i in range (len(file_list)):
        ...
        if time_gap >= cleanperiod: # 파일을 남겨둘 최대 기한을 넘기는 파일을 발견한다면, list에 추가
            data_path_list.append(raw_path+'/'+file_list[i])
            #print(file_list[i])
    return data_path_list

def file_clean(aename, cleanperiod):

    data_path_list = delete_filepath_list(aename, cleanperiod) 
...
def doit():
    for aename in ae:
        cleanperiod = ae[aename]["config"]["cmeasure"]["rawperiod"]+60 #rawperiod보다 1분 더 여유를 두고 삭제 작업을 진행
